...
def define_computation_graph(source_vocab_size: int, target_vocab_size: int, batch_size: int, is_training: bool=False):
    ...
    with tf.name_scope('Bi-Encoder'):
        ...
        # define dropout for encoder cells
        if is_training and C.DROPOUT < 1:
            encoder_cell_fw = tf.contrib.rnn.DropoutWrapper(
                encoder_cell_fw, output_keep_prob=C.DROPOUT)
            encoder_cell_bw = tf.contrib.rnn.DropoutWrapper(
                encoder_cell_bw, output_keep_prob=C.DROPOUT)
        ...
        bi_outputs, bi_encoder_final_state = tf.nn.bidirectional_dynamic_rnn(
        ...
        encoder_final_state = tf.concat(bi_encoder_final_state, -1)

    # with tf.name_scope('Multi-Bi-Encoder'):
    #     output = input
    #     for n in range(C.NUM_LAYERS):
    #         # define forward cell (use only half of length due to bidirectionality)
    #         encoder_cell_fw = tf.nn.rnn_cell.GRUCell(C.HIDDEN_SIZE/2)
    #
    #         # define backward cell (use only half of length due to bidirectionality)
    #         encoder_cell_bw = tf.nn.rnn_cell.GRUCell(C.HIDDEN_SIZE/2)
    #
    #         initial_state_fw = encoder_cell_fw.zero_state(
    #             batch_size, tf.float32)
    #         initial_state_bw = encoder_cell_bw.zero_state(
    #             batch_size, tf.float32)
    #
    #         bi_outputs, bi_encoder_final_state = tf.nn.bidirectional_dynamic_rnn(
    #             encoder_cell_fw, encoder_cell_bw, encoder_inputs_embedded, initial_state_fw=initial_state_fw, initial_state_bw=initial_state_bw, scope='Encoder_Layer_'+str(n+1), dtype=tf.float32)
    #
    #         encoder_outputs = tf.concat(bi_outputs, -1)
    #         encoder_final_state = tf.concat(bi_encoder_final_state, -1)
    ...
    with tf.variable_scope("Decoder"):
        ...
        # define dropout for decoder cell
        if is_training and C.DROPOUT < 1:
            decoder_cell = tf.contrib.rnn.DropoutWrapper(
                decoder_cell, output_keep_prob=C.DROPOUT)
